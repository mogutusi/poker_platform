...
def _broadcast_samples() -> list[S.ServerMessage]:
    ...
    return [
        ...
        S.HandEnded(winnings=(S.NickAmount(nickname="A", amount=3),), refunds=(),
                    stacks=(S.SeatStack(seat_position=0, nickname="A", points=101),)),
        ...
        S.DMRead(reader_nick="B", read_through=_NOW),
...
def test_client_registry_covered_by_to_command():
    ...
    samples = {
        ...
        C.DMMarkRead: C.DMMarkRead(peer_nick="bob", read_through=_NOW),
    }
    ...
    for cls, msg in samples.items():
        if cls in (C.JoinRoom, C.FetchRoomChat, C.DirectMessage, C.DMMarkRead):
            with pytest.raises(AssertionError):  # JoinRoom 走 DB 富化 / FetchRoomChat·DirectMessage·DMMarkRead 走 shell 路由,均不走 to_command(契约)
