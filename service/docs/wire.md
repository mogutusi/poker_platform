...
- **共享词汇目录**(非消息,但同走「单源 + codegen」):**表情目录已落地**([0035](refactor/changes/0035-emoji-implementation.md))——`app/wire/emoji.py` 的 `EmojiCode` 封闭枚举 + `EMOJI_CATALOG` → 生成器 `_emit_emoji_catalog` **无条件吐** `EmojiCode`/`EmojiMeta`/`EMOJI_CATALOG` 进 `wire.gen.ts`(因不被任何消息引用,不走 `_discover` 的 ref_set 断言路径),供前端渲染 `[code]`(后端纯透传、不进报文,见 [messaging.md](messaging.md)「表情」)。漂移由 `test_codegen_uptodate` + `test_emoji` 兜。
