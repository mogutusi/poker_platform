...
## 问答补记(push 后用户提问,2026-07-13)

**问**:新密钥怎么拿到?「管理员终端 stdout」是什么?更新密钥是自动的吗?——**结论**(已核对均在 auth.md/dev.md/CLI 头注,唯 crontab 具体写法只在对话里 → 本次补进 dev.md):新钥 = 管理员跑 `rotate`/`issue` 时打印在其屏幕上的那一行,这是密钥**唯一导出点**(不进服务器日志/文件/信道);**换钥半自动**(cron 周期跑、幂等)、**发钥永远手动带外**(设计使然,自动下发会被旧钥链式解出);忘跑 cron 不锁人(`k_cur_until` 只是排程)+ 旧钥有宽限。dev.md 补 crontab 示例 + 输出文件卫生(600 权限、发完即清)。

**追问**:这(半自动语义)不该写进 auth.md 吗?——**对**:「换钥半自动、发钥永远手动」是设计语义,归 auth.md;此前散在三段(不做进程内调度/下发带外/别用信道下发决策)没有一句定性,且 0066 改写时把原文「(实现选一,见 dev.md)」的 dev.md 链丢了。→ auth.md §轮换任务 补「一句话定性」段 + 回链 dev.md「K_user 管理」(crontab 示例留 dev.md——设计文档讲设计、运维操作归 dev.md 的既有分工)。
