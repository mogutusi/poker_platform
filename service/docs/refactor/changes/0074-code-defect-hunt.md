# 0074 · 第六轮:纯代码缺陷猎杀(换靶——不再查文档一致性)

日期:2026-07-29 · 性质:**缺陷猎杀 + 即修**(与 0072 的「符合性审计」互补:那轮问「代码是否守文档」,本轮问「代码本身是否会错」)· 触发:用户「继续发现代码问题」。

## 与前五轮的分工

0072 五轮工作流把**文档↔代码一致性 + 架构合理性**挖到收敛(8 条原发现 + 36 条新发现全部对抗验证)。本轮**换靶**,只打纯代码缺陷(crash / correctness / security / resource-leak / data-loss / logic-bug),且专攻此前覆盖最薄的面:
...
### 0074·B · `_disconnect` 令投票人转 OFFLINE 却不重算免盲投票(medium · logic-bug)

- **机理**:`_voters()` 要求 `READY_TO_PLAY`,故掉线者转 `OFFLINE` **即退出合格投票人集**——与「离场/坐出/起身」同为「投票人减员」。但 [reduce.py `_disconnect`](../../app/core/reduce.py) 的在座者臂只标 OFFLINE + 广播,**未调 `_maybe_resolve_entry_vote`**;而 `_begin_leave`(观战者臂/离场)与 `_set_user_status`(坐出/起身)都调了。
- **后果**:靠减员达成的全票**永不通过**——投票悬挂到 `_start_hand` 处 `room.entry_vote = None` 被静默作废,新玩家该免的盲照付。违反 rules.md ①.15「投票人集合缩小后重算」。
- **主审计者对照实验**(同一初始状态、同一语义事件、两条路径):
  - bob **坐出** → `entry_vote=None`、`waive_entry_for={'carol'}`(投票通过,carol 免盲)
  - bob **掉线** → `entry_vote` 仍悬挂、`waive_entry_for=set()`(免盲丢失)
- **修复**:`_disconnect` 在座者臂末尾 `*_maybe_resolve_entry_vote(work, room)`(与既有两处挂钩点同形,一行)。测试 `test_voter_disconnect_recomputes_and_passes`,**变异验证**:还原旧 return → 该测必红。

**测试基线**:712 → **714 全绿**(两条各 +1)。
