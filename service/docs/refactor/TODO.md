...
- [ ] **0082·A** `new_here` 缺传达渠道:它只在 `StateSnapshot` 的 `SeatView` 里,`_start_hand` 末尾重标时不发事件,前端那份打完一手就过期 → 无法准确判断免盲开票入口该不该显示(当前做法是不预判、交服务器裁决)。考虑让 `UserStatusChanged` 带 `new_here`,或重标后补广播
- [ ] **0082·B** 免盲投票走完整流程(全票通过 → 新人免盲入局)的验证;房间配置的买入额与越界拒绝
