...
- [x] 前端消费 wire.gen.ts — **已完成**(store 自 0078 起即用 `wire.gen.ts`;`poker.ts` 的手写协议残留于 [0099](changes/0099-retire-the-mockup-types.md) 删除)。原文如下,留作当时的判断:~~**延后,随前端 WS client 集成**~~:0017 已生成 `wire.gen.ts` 解锁前端按真类型写 WS client;但 `frontend/src/types/poker.ts` 是 **UI mockup 聚合类型 + 本地 mock 牌局逻辑**(非协议类型),且本批无 `Player`/`StateSnapshot` wire 类型——此刻删它只会破坏 mockup 无替代。删 poker.ts + 改组件归「前端 WS client + StateSnapshot」单元(见 0017 决策 8)
...
- [~] **0072·D2-D5** 文档 truth-up 一批(仿 0047/0067):~~D1 messaging.md §房聊历史残留旧段~~(0075 连带解决)/ ~~D2 architecture.md 不变量 2 补只读豁免判据~~(**0092 已修**:改写成「本体是唯一写者」+ 三条判据 + 现存三处名单 + 新增豁免的论证要求)/ ~~**余**:connection.md·lobby.md 待定段陈旧(D3)/ 四处陈旧注释含两处 JWT 反事实(D4)/ 小项(D5)~~ —— **0100 全部做完**(D4 的 JWT 反事实实为 6 处;D3 最离谱的一处是「动态建房仍待定」写反了)
...
- [x] **0072·N5** `SessionStore.revoke` 全仓零调用者 — **0097 已修**:`POST /user/logout`(前端「退出」真的调它了)+ 改密吊销该账号其它会话 + `revoke` 就地判死 `Session` 对象(**登记没说的结构性缺口**:只 pop 表项挡不住已经连着的人,收发两侧只比对 `conn.session.expires_at`、从不查表)。**登记的「建 name→sessions 索引」不需要**(`rename_nickname` 同款线性扫,≤20 在线);**`issue --reset` 那半架构上做不到**(CLI 是独立进程,够不着服务器内存里的会话表),已改为在 auth.md 如实记档「唯一手段是重启」 — 详见 [BUGS.md#BUG-8](BUGS.md)
...
- [~] **0072·N-低危设计边角**(low,择机)—— **0092 做掉三条 + 0095 做掉 N-e10/N-e11 + 0096 做掉 N-e16**(~~N-e26 孤儿脚本~~ 已删、~~N-e36 `_NICKNAME_MAX_LEN` 二份事实源~~ 已改引 schema、~~C3 `big_blind=2*`~~ 已改引常量、~~N-e16 `_evict` 不清 `waive_entry_for` 致离房重进免盲~~ **0096 已修**:`_evict` 剔除该 nick,回归测钉到筹码面)。**余**:~~N-e9 DM 游标无单调防护~~(**0098 已修**:唯一写者处只前进;登记漏了的「指向未来」那半一并钳,那半会致私信永不补收并被保留清理真删)/ N-e34 NullPersister 无生产消费者 / N-e35 Presence 三方法零消费者 / N-e38·N-e40 演进面与快照 min-raise 记档
