...
- [x] P4 三之二 DB-backed dev shell(0029):`DevShell.setup()` async engine + `create_all` 建表 + 幂等种子 dev 用户进 DB + 从 DB 载入积分建 world + `OrmPersister` 替 `NullPersister` + 关闭 `engine.dispose()`;端到端冒烟(命令穿 gameloop → reduce → Persist → OrmPersister → 真 DB 行:买入 PointsWrite UPDATE / 一手牌 HandRecord+participants INSERT;test_dev_db_e2e 4,共 270)。**余 0030(三之二-c)per-join wire-load**:wire `client.JoinRoom`(`join_room{room}`)+ Receiver 按 nick 读 DB 构 `JoinRoom(room,uid,loaded)` + dev 改「连接进大厅 → 主动 join_room」(退役启动整载)+ 重 codegen
