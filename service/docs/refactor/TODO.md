...
- [ ] 聊天表情(emoji,设计见 [changes/0034](changes/0034-emoji-catalog-design.md)):`app/wire/emoji.py` 封闭目录(`EmojiCode` + `EMOJI_CATALOG{label,glyph}`)+ `gen_wire_ts.py` 无条件吐目录 + 前端按 `[code]` 渲染(后端纯透传、`ChatMessage`/`_room_chat` 不变)+ 起始目录定稿 + 测(codegen 漂移 / 目录非空 / code 形如 `[a-z0-9_]+`)。**仅前端渲染约定 + 共享目录,无新协议字段**;房聊现可用、私聊落地后自动适用
