...
- [~] P4 delayDB:0024 落地 `WriteBuffer` 双缓冲(状态写按键覆盖 / 事件写追加 / `put` 单入口 `_state_key` 分流 / `swap` / `requeue` 更新者优先;test_persist 12);0025 落地 `PersistWriter`(`Persister` 协议 + `NullPersister`;`flush_once` 先 swap 后 await / 失败回灌 / 毒丸丢批 / `drain` 有界 + 节流;gameconfig DB 旋钮;接进 DevShell start/stop;test_persist_writer 11,共 252);**`to_orm` + `db/` ORM 模型(`User` 加 uid/积分、`HandRecord`+`Participant` 对齐 `HandRecordWrite`)+ Alembic 迁移 + 真 `OrmPersister` 替 `NullPersister` + 载入(Receiver 读 DB)待 P4 三(需 DB 基建)**
