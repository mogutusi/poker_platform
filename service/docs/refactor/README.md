...
> **执行顺序已重排(0016,前端解锁)**:上表是**职责地图**,不是执行序。因前端要联调,**wire 协议(P6)+ 一个明文 dev 端点(P2/P3 最小版)前置**到 P1 主体之后先做,**国密安全信道(P5)推到最后**,**协议按模块增量交付**。实际执行序与详细拆分以 [TODO.md](TODO.md) 的「执行顺序」+ [changes/0016](changes/0016-replan-wire-first.md) 为准;本表的分层/产出/参考不变。
