...
### ~~BUG-20 · 座位上显示的筹码从入座那一刻起就不再变~~ —— **0106 已修**

- **来源**:[0105](changes/0105-the-showdown-you-can-actually-see.md) 的对抗复审(**浏览器实测**,不是推断)· **修于 [0106](changes/0106-seat-chips-finally-follow-the-hand.md)**(在手那半:座位投影照抄服务器快照口径「在手取 `players[].points`,不在手取 `seats[].points`」;结算那半:`HandEnded` 加 `stacks`——各参与者结算后的座位筹码,与手牌记录 `final_points` 同循环同源,含本手离桌者)
- **登记的「要补的测试」有一处不精确,0106 已按真关系写**:「赢家的座位增加了他赢到的数」相对**开局前**的座位值不成立(差了投进池子的部分);成立的关系是 `结算值 == 下注扣完后的显示值 + winnings + refunds`,e2e 正是这么断言的(三个数全是服务器给的)。
- **第二条测试义务(「打光筹码之后买入入口要出现」)的落点**:store 测试钉住了数据前提(`stacks` 送 0 → `mySeat().points === 0`,这正是买入输入框与 Ready 禁用两道门读的数);**门本身的 JSX 渲染没有自动化钉**——真打光要输掉一手 all-in,浏览器里造不出确定性的局。缺口如实记在 0106 ⑥。
- **定级说明(留档)**:它不符合本篇 medium「特定交错或边界下才触发」那句——它是**无条件的**,每一手都错。定 medium 的理由是后果:纯展示失真,不丢数据、不影响服务端结算,且重连/重进房就会被快照修正。
...
- **修法(择一)**:① 给套件一个前置钩子,跑前把 dev 账号拉回种子值;② 用例结束时归还筹码(现在「结束时还停在手牌里」的那几条本来就还不掉);③ 至少把失败信息改成能自解释的。**③ 已于 [0107](changes/0107-the-suite-says-why-it-cannot-buy-in.md) 做掉**(`joinTable` 在 Ready 不可点且界面上挂着服务器拒绝时,抛出带 `detail`(如 `have=50 need=100`)+ BUG-21 指引 + 恢复法的错误;真拿一个调到 50 分的账号验过报错文本)。**①/② 未做,漏水的机制还在**——本条不划掉,套件仍会随轮次流失筹码,只是红的时候不再骗人。① 要做对得走「dev 专用端点或重启后端」(直改 DB 会跟内存权威打架),是独立的测试基建决定。
...
| ~~BUG-15(N-e34)~~ | ~~`NullPersister` 无生产消费者~~ —— **0102 改判结案,不删**:它有两个**测试**消费者,是 `Persister` 协议的空实现/测试替身,「只被测试用」不是死代码。**真正的缺陷是文档说假话**(`db.md` 与 `persist.py` 头注都写「dev 用 NullPersister」,而 0029 起 `DevShell` 无条件用 `OrmPersister`),那两处 0100 已改实 ⇒ 本条实际已修完 |
| ~~BUG-16(N-e35)~~ | ~~`Presence` 的三个方法零消费者~~ —— **0102 已删**(`is_online`/`room_headcount`/`online_nicks`),留下唯一有生产调用的 `current_room`。删前先答了「有没有谁本该用它」:**没有**——lobby 要的是 seated/watching 两个更细的量,messaging 要的是连接对象本身,`online_nicks` 是纯转发。删完 `Presence` 不再需要 `ConnectionManager`,签名收成 `Presence(world)` |
...
| 0105(BUG-20)| 座位筹码从入座起不再变:在手实时值没进座位投影 + 结算值 wire 上无承载 | 0106 |
