...
**关旧 ws 必须带 `WS_CLOSE_DISPLACED`(4409)**,不能用普通关闭(0087)。「静默」是指对 `world` 与房内其他人静默,不是对**被顶的那个客户端**也不给交代:它若只看到一次无码关闭,就与掉线无从分辨,于是照常退避重连——而它一重连就把刚上位的那条顶掉,对方再重连,两边无限互顶(浏览器里实测 6 秒 6 轮)。带上码,客户端才知道该停手并提示「账号在别处登录」。关闭码全集见 `shell/connection.py` 的 `WS_CLOSE_*`,前端契约见 [frontend/BACKEND_GUIDE.md](../../frontend/BACKEND_GUIDE.md) §4.3。
