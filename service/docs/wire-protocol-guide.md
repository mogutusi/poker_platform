...
一句话:平时只翻自己的牌,摊牌才翻对手,翻开之后留到下一手开始(界面侧的结算展示期,见 [frontend/docs/state.md](../../frontend/docs/state.md))。
...
一句话:只有一个 ws 端点 `/ws?sid=`,逐帧加密;明文 dev 端点已随 0086 退役。载荷 JSON 与加密层无关,`switch(type)` 一字不改。
