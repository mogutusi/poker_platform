...
2. **`world` 只由 GameLoop 经「工作副本 commit」更新**:`reduce` 只改副本,**写**永远只有这一条路;其它协程只读已提交状态。
   - **不变量的本体是「唯一写者」,不是「谁都不许读」。** shell 里有一族**记档合规的只读豁免**:同步读一眼已 commit 的 `world` 做纯投影,不写、不据它决定要不要载入。判据三条:①只读、②读的是已 commit 的状态、③全程同步(无 `await`,所以读到的一定是某条命令的完整结果,不会撞见半改)。现存三处:presence([0037](refactor/changes/0037-presence.md))、`POST /lobby/rooms`([0048](refactor/changes/0048-rest-lobby-rooms.md),0094 起走信封)、`FetchRoomChat`([0071](refactor/changes/0071-room-chat-history-in-room.md))。
...
- 全局串行:所有房间共用一个 GameLoop,存在跨房间的队头阻塞。本规模可忽略。房间多时可改为每房间一个 GameLoop——**core(reduce)确实不变,但 shell 的迁移面比「换个循环」大**(0072·N-e38 记档):①`checkout` 把**整份** `users` 表深拷进每条命令的工作副本、`commit` 整份替换,工作副本模型本身假设 `users` 只有一个写者,分房循环 = 多个写者赛跑同一张全局积分表;② 命令不带 room(模型 2),目标房由 `world.users[origin].room` 解析,分房后这次解析要发生在进任何循环**之前**,是一次落在所有工作副本之外的跨房读;③ 只读豁免三处与 Timer、dispatch 都持同一个 `world` 引用,分房后每处都要回答「读哪份」。
