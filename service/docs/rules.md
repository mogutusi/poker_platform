...
- **位次**:button/盲注在已入局座位间轮转;选"等大盲免费"的人,按其座位被 BB 路过时入局(精确判定属实现细节,见下「实现细节」)。
- **默认**:坐下并 ready 即视为选"付盲即玩";要免费等就显式设"等大盲"(`SitDown.wait_for_big_blind`,见 [wire.md](wire.md))。
- `new_player_seat_list` 不需要(本方案不依赖死盲记账)。

**实现细节(0023,reduce)**:

- **「上一手是否参与」即 `new_here`(防躲盲核心)**:`_start_hand` 末尾把**本手未被发牌的每个在座者**置 `new_here=True`。这把行 41/50 的「判据是上一手是否参与」落成一条统一规则——换座/退房再进经 `SitDown` 本就新建 `new_here=True` 座位;**坐出再回 / 没 ready 干等一手 / 断线跨手**则靠这条重标兜住(玩家全程没碰 `SITTING_OUT` 也躲不掉)。被发牌者本手算参与、保持 `new_here=False`(连续每手玩的人永不被重标,不过度收费)。重标在 eligibility/bootstrap 读 `new_here` **之后**跑,故不自扰本手发牌集。
- **等大盲入局时机 = 成为大盲那手(`blinds.sweep_entrant`)**:庄位**只在 `core_dealt`(established/付盲即玩/bootstrap/waive 的发牌集,不含等大盲者)上推进** ⇒ 等大盲者永不持庄/小盲,只可能当大盲。某等大盲者当且仅当 `seat_order(button, core_dealt ∪ {它})[1] == 它`(它正好是大盲)才本手免费入局,**下的那个结构大盲即入局费、不额外 post**。多个等大盲者同手都「单看会是大盲」时,真正入局的唯一大盲 = `core_dealt ∪ 全候选` 的 `order[1]`(最靠小盲那个),其余下手随庄推进再轮到(不饿死、不瞬移)。这解掉「谁入局 ↔ 谁是大盲」的循环依赖,且 heads-up core(2 人)接纳一个等大盲者翻成 3 人时位次仍走标准 `seat_order`(无 waiter 当小盲/庄)。
- **waive 优先于等大盲**:`waive_entry_for`(免盲投票通过)在 eligibility 里先于 `wait_for_big_blind` 判 → 被免盲者进 `core_dealt` 免费正常入局,不走等大盲 sweep 路径。
- **残留简化(可接受,本规模)**:① **空 core 停摆**——唯一 established 全坐出/断线、只剩 `READY` 的等大盲者时,`core_dealt` 空 → `NOT_ENOUGH_PLAYERS` 等待(等大盲者单独不能 bootstrap,守「无免费空降好位」),任一 established 再 ready / 来个付盲者即自愈。② **单 established + 单等大盲者**打 heads-up 时,后者照样下一个**真实结构大盲**(结构大盲即入局费,最反直觉但正确)。③ **任何原因错过一手都收入局费**(坐出/断线/没 ready,无意图区分)——这正是防躲盲 airtight 的根因。
