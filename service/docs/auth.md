...
- **服务器 exp 兜底**:`session_token.exp` 到点,服务器**拒绝该会话**——新握手/REST 查表即拒(0055/0062),**活 ws 连接亦强制**(0070:收帧/出站各比对一次 `expires_at`,过期关连接 4401;双向零流量的过期连接存活到下次任一方向活动,记档接受——无流量即无泄露面)。正常情况下客户端已提前轮换,不会撞到。
