...
- **结算**:每个 `Player.points`(赢得的 + 剩余)还回 `Seat.points`,`Seat.in_game_points=0`。`PLAYING` 玩家 UserStatus → `SITTING_IN`。局中请求坐出者(`room.sitting_out_next`)转 `SITTING_OUT`。局中离桌者(`room.leaving`)不转状态,随后驱逐。
