...
| `Connect(nick)` | None | shell | 握手后接入**大厅**(不带 room/积分)。若 nick 已在 `world.users`(在某房、之前 OFFLINE)→ 重连恢复 + 私发 `Personal(StateSnapshot)`;否则纯大厅接入,core 无事(见 [connection.md](connection.md)) |
...
| 买入/离桌 | `Broadcast(...)` + `Persist(PointsWrite)` | — |
