...
mod32 = uint32(0xFFFFFFFF)


IV = np.asarray([0x7380166f, 0x4914b2b9, 0x172442d7, 0xda8a0600, 0xa96f30bc, 0x163138aa, 0xe38dee4d, 0xb0fb0e4e], np.uint32)
    ...
    for i in range(0,len(B_),16):
        ...
        for j in range(16):
            if j<12:
                ...
                W[j+4] = i8_u4( temp0^(temp0<<15|temp0>>17)^(temp0<<23|temp0>>9) ) ^ i8_u4( W[j-9]<<7|W[j-9]>>25 ) ^ W[j-2]
            ...
            E,F,G,H= i8_u4(temp1^(temp1<<9 | temp1>>23) ^(temp1<<17 | temp1>>15)) , E, i8_u4(F<<19|F>>13) ,G
...
def sm3_hash_bytes(msg:bytes|str) -> bytes:
    if isinstance(msg,str):
        msg = bytes(msg, encoding='utf-8')
    pad_num = 64 - ((len(msg) + 1) & 0x3f)
    msg += b'\x80' + (len(msg) << 3).to_bytes(pad_num if pad_num >= 8 else pad_num + 64, 'big')
    V =CF(np.copy(IV), np.frombuffer(msg, np.uint32).byteswap()).byteswap().tobytes()
    return V

def KDF_sm3(key:bytes,length:int):
    if length > 32:
        raise ValueError("length > 32")
    return sm3_hash_bytes(key)[:length]
