...
_SM4_S_BOX = np.asarray([
    ...
    0x18, 0xf0, 0x7d, 0xec, 0x3a, 0xdc, 0x4d, 0x20, 0x79, 0xee, 0x5f, 0x3e, 0xd7, 0xcb, 0x39, 0x48],np.uint8)
_SM4_TBOX0 = np.asarray([0x8ED55B5B, 0xD0924242, 0x4DEAA7A7, 0x06FDFBFB, 0xFCCF3333, 0x65E28787,
    ...
    0x8BD45F5F, 0xE7C82F2F, 0xDD39E4E4, 0x68492121],np.uint32)
_SM4_TBOX1 =np.asarray([0x5B8ED55B, 0x42D09242, 0xA74DEAA7, 0xFB06FDFB, 0x33FCCF33, 0x8765E287,
    ...
    0x5F8BD45F, 0x2FE7C82F, 0xE4DD39E4, 0x21684921],np.uint32)
_SM4_TBOX2 =np.asarray([0x5B5B8ED5, 0x4242D092, 0xA7A74DEA, 0xFBFB06FD, 0x3333FCCF, 0x878765E2,
    ...
    0x5F5F8BD4, 0x2F2FE7C8, 0xE4E4DD39, 0x21216849],np.uint32)
_SM4_TBOX3 =np.asarray([0xD55B5B8E, 0x924242D0, 0xEAA7A74D, 0xFDFBFB06, 0xCF3333FC, 0xE2878765,
    ...
    0xD45F5F8B, 0xC82F2FE7, 0x39E4E4DD, 0x49212168],np.uint32)
...
def _T_key(ka):  # 用于生成圈密钥的T'变换
    B = (_SM4_S_BOX[ka >> 24] << 24)| (_SM4_S_BOX[(ka >> 16 )& 0xff]  << 16) | (_SM4_S_BOX[(ka >> 8) & 0xff]  << 8) | _SM4_S_BOX[ka & 0xff]  # S盒变换
...
@njit(uint32(uint32), nogil=True, cache=True)
def _T(x):  # T变换
    return (_SM4_TBOX0[x>>24]) ^ (_SM4_TBOX1[(x>>16)&0xFF]) ^ (_SM4_TBOX2[(x>>8)&0xFF]) ^ (_SM4_TBOX3[x&0xFF])
...
class SM4:
    ...
    def ecb_enc(self, data):
        l = len(data) % 16
        padding_value = 16 - l if l > 0 else 16
        padding_bytes = bytes([padding_value] * padding_value)
        data += padding_bytes
        return array2byte(_ecb_base(self.e_rk, byte2array(data)))

    def ecb_dec(self, data):
        ret = array2byte(_ecb_base(self.d_rk, byte2array(data)))
        num = int(ret[-1])
        return ret[:len(ret) - num]
    ...
    def cbc_dec(self, iv, data):
        ret = array2byte(_cbc_dec(self.d_rk, byte2array(iv), byte2array(data)))
        num = int(ret[-1])
        return ret[:len(ret) - num]

def sm4_cbc_enc(key:bytes, iv:bytes, data:bytes) -> bytes:
    return SM4(key).cbc_enc(iv,data)

def sm4_cbc_dec(key:bytes, iv:bytes, data:bytes) -> bytes:
    return SM4(key).cbc_dec(iv,data)

def sm4_ecb_enc(key:bytes, data:bytes) -> bytes:
    return SM4(key).ecb_enc(data)

def sm4_ecb_dec(key:bytes, data:bytes) -> bytes:
    return SM4(key).ecb_dec(data)
