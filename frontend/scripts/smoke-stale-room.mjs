...
//
// 制造残留:smoke1 进 A 房、入座、然后直接断线(在座断线 → 后端保留座位)。
...
import { Client, login, sleep as wait } from './smoke-client.mjs'
...
console.log('① 制造残留:smoke1 进 A 房、入座、直接断线')
{
  const s = await login(USER)
  const c = new Client(USER, s)
  await c.connect()
  ...
  check(!!seated, `smoke1 已在 ${ROOM_A} 入座`)
  c.ws.close() // 在座断线:后端按不变量 9 保留座位
  await wait(400)
}
