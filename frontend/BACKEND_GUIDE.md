...
后端是**单进程、内存权威**的状态机。理解下面四句话,大部分接口行为都能推出来:
...
4. **断线不丢状态,重连拿快照**。你掉线后座位和筹码保留一段时间(默认 90 秒);**在房间里**的用户重连成功,服务器会私发一条 `state_snapshot`,一次性给你整桌当前状态。**前端不需要事件回放/补历史**——任何时候拿到 `state_snapshot` 就把桌面整个重建一遍。注意:**在大厅重连不会收到任何消息**(大厅没有状态可对齐),别把"等到快照"当成重连完成的信号——连接建立成功即算完成,快照只在你身处房间时才来。
