...
服务器不走 TLS,而是在**应用层**用国密三件套自建加密信道。前端要自己实现这一层。**这一层绕不过去**——曾经用来先跑 UI 的明文 dev 通道已经移除(§4.5),现在登录、ws、需身份的 REST 全都走加密。但它不难走:后端生成的测试向量(§4.1 末)逐条守着填充/字节序/域字节这些坑,本仓的 TS 实现是照着它写出来的。
...
**服务器主动关连接的三个码,处置完全不同**(0087 定齐):

| close code | 什么意思 | 你该做什么 |
|---|---|---|
| **4400** | 帧伪造 / 重放 / 损坏(`FrameError`)| 别重连去撞。查密钥域(ws 用 0x01/0x02)、查 seq 有没有回退 |
| **4401** | 握手鉴权失败;活连接撞上会话 `exp`;**或会话被吊销**(自己登出 / 在别处改了密码,0097)| 重新登录换会话。三种来源同码,客户端不必区分 |
| **4409** | **同账号的新连接接管了你这条**(顶替)| **绝不自动重连**。会话本身还有效,但这条连接已让位——一重连就把对方顶掉,对方再重连,两边无限互顶(实测 6 秒 6 轮)。提示用户"账号在别处登录" |

其余关闭码(1000/1006 等)都按普通掉线处理:退避重连,重连后 `state_snapshot` 会补齐一切。
...
### 4.5 曾经的开发期捷径:明文通道(**已移除**)

后端一度开着一个不加密的 dev ws 端点 `/dev/ws?nick=`,用来在加密层写完之前先跑通牌局 UI。**它已经没有了**(0086):前端加解密早已落地(29/29 向量 + 三条冒烟 + 浏览器用例全走加密),既定的退役条件满足,而它无鉴权——握手只看 `?nick=` 是不是 dev 用户名,谁连上都能冒充那个身份。

所以现在**只有一条路**:`POST /user/login` 换会话 → 连 `/ws?sid=`,逐帧加密。加密层不再是可以推后的一步,§4.1–4.4 必须先跑通。好消息是这一层有后端生成的测试向量(`crypto-test-vectors.json`)逐条守着,不用靠猜。
...
start_hand 后你会收到:
  ...
  → hand_show_down(摊牌,揭示未弃牌者的牌)→ hand_ended(结算;`stacks` 带各参与者结算后的座位筹码)
```
...
- **座位筹码别自己记账。** 手内的实时筹码逐街来自 `hand_status_changed.players[]`,每次行动由 `player_acted.points`(只有行动者本人的)更新,座位卡片显示按「在手取 `players[].points`,不在手取 `seats[].points`」合并——这正是服务器快照 `seats[].points` 自己的口径。手尾的结算值由 `hand_ended.stacks` 直接给(各参与者结算后的座位筹码,含本手离桌者——他的 `user_left` 随后就到),**别拿 winnings/refunds 去加**:那是在复算结算的记账,而且相对开局前的座位值根本不成立(差了投进池子的部分)。0106 之前 `hand_ended` 不带这个数,座位数字从入座起一动不动(BUG-20)。
...
- **私信已读游标只前进,而且不会超过服务器此刻**(`dm_mark_read.read_through`)。往回送一个更早的值服务器不采纳,送一个未来的值会被钳到「现在」——两种都**不报错**,因为游标是幂等的进度上报、不是命令。所以 `dm_read` 回执里的 `read_through` **可能小于你送的值**:那是服务器实际采纳的进度,以它为准,别拿自己送出去的那个数当真相。带时区偏移的时间戳也没问题(服务器统一换算成 UTC),照抄 `dm_delivered.created_at` 最省事。
...
| `POST /user/password` | `{old_password, new_password}` → `{status}` | 改密码要验旧密码;403 = 旧密码错。**改成功会吊销你在别处的会话**(当前这个留着)|
| `POST /user/nickname` | `{new_nickname}` → `{status, nickname}` | **只能在大厅改**(在房间里 403);撞名 409 |
| `POST /user/logout` | `{}` → `{status}` | 登出:吊销**当前这一个**会话。别的设备不受影响 |

**登出必须调服务器**(0097):只清本地状态不算登出——服务器上那把 `session_token` 会一直有效到 `exp` 自然到期,谁拿到都还能收发。调用顺序上注意:封包要用到当前会话,所以 `POST /user/logout` 必须在清本地会话**之前**发出。调用失败(断网等)不该卡住本地登出,那把 token 最坏也就活到自然到期。

**会话被吊销之后**(自己登出、或在别处改了密码):这个 `session_id` 的后续 REST 一律 401,而**已经连着的 ws 会在下一帧收发时被关闭,close code 4401**——与撞上 `exp` 同一个码、同一条路径。看到 4401 一律当「必须重新登录」处理即可,不需要区分是过期还是被吊销。
