...
- 服务器主动关连接的三个码,处置各不相同:
  - `4400` 帧被判伪造/重放/损坏 —— 别重连去撞,先查密钥域和 seq。
  - `4401` 鉴权/会话过期 —— 引导用户重新登录,不要自动重连。
  - `4409` **被同账号的新连接接管**(顶替)—— **绝不自动重连**,见下「重连」。
- 其余关闭码(1000/1006…)都是普通掉线,退避重连即可。

### 重连

- 网络断开是常态,自动重连,退避重试。
- 重连成功后服务器会私发一份 `StateSnapshot`,前端整份替换本地状态即可,**不需要自己补状态**。
- 同一账号在别处登录会「顶替」当前连接:旧连接被关掉,关闭码 `4409`。**这时不要自动重连去抢**——两边都重连就是无限互顶(0087 在浏览器里实测过 6 秒 6 轮),提示用户「账号在别处登录」即可。
