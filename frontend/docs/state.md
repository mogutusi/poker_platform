...
`HandShowDown` 与 `HandEnded` 是服务端**同一批**产出的事件(`_settle_and_end` 一次返回),两条报文前后脚到达。所以「手牌一结束就把牌面收掉」等于摊牌根本没被看见——0105 实测是**6 毫秒**。

约定因此是:**`HandEnded` 之后,公共牌与已亮出的底牌继续留在桌上,直到下一手开始。**

- 判据是 `reveals` 非空,不另立状态、也不起计时器:它只由 `HandShowDown` 写入,而清空它的只有 `HandStarted`、`StateSnapshot` 和离房重置(`resetRoom`)——三者都意味着「没有上一手可看了」。所以「非空」精确等于「这一手摊过牌且新一手还没开始」。
- **`HandStarted` 那次清空从此是隐私红线,不只是整洁**:漏清的话,上一手对手的底牌会在**整局新牌**里一直朝上。在摊牌还渲染不出来的年代漏清是看不见的,现在看得见了,所以 `room.test.ts` 有两条用例分别钉住 `HandStarted` 与 `StateSnapshot` 这两次清空。
- **底池不留**:钱已经付出去了,继续显示会让人以为桌上还有筹码;赢了多少由结算面板说。
- **没摊牌就没有展示期**(有人弃牌收尾的那种):`reveals` 是空的,没有任何别人的牌可亮。
- **展示期中途重连会丢**:`StateSnapshot` 清 `reveals`,因为服务器不保存已结束手牌的摊牌。前端**不许**为此自己留一份——那就是凭空造真相。

这条不违反不变量 1:留在屏幕上的每一张牌都是服务器发过来的,前端只是没有提前把它擦掉。展示窗口本身属于「[UI 局部状态](#ui-局部状态归谁)」那一类。
...
- **前端自己的**:加注滑块当前值、面板开合、动画进度、输入框内容、乐观的「按钮已点击、等回执」置灰,以及**服务器给的东西在屏幕上留多久**(如[结算展示期](#结算展示期摊牌要留在桌上)——留的是服务器发过的原样,只是不提前擦掉)。
